...
-- Apply in Supabase SQL Editor if skilled players are rejected after quick genre tests.
...
begin
  ...
  for value_text in select jsonb_array_elements_text(p_answer_times) loop
    if value_text ~ '^[0-9]+(\.[0-9]+)?$' then
      value := round(value_text::numeric, 1);
  ...
  if count_values < 10 then
    return false;
  end if;

  -- Previously 0.35s; allow very fast visual-recognition questions such as silhouettes.
  if sum_values / count_values < 0.25 then
    return true;
  end if;

  -- Uniform timings are suspicious only when they are extremely uniform.
  if max_value - min_value <= 0.1 and sum_values / count_values < 0.7 then
    return true;
  end if;
  ...
  if coalesce(repeated_count, 0) >= 20 then
    return true;
  end if;

  for period in 1..5 loop
    if count_values >= period * 4 then
      is_periodic := true;
      for index_value in (period + 1)..count_values loop
        if abs(timing_values[index_value] - timing_values[index_value - period]) > 0.05 then
          is_periodic := false;
          exit;
        end if;
      end loop;
